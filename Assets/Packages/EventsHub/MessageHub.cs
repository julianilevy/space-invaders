...
namespace IntrovertStudios.Messaging
{
	...
	public class MessageHub<TKey> : IMessageHub<TKey>
	{
		...
		{
            List<Action> connections = GetConnectionList(id).ToList();
		...
		{
			List<object> connections = GetTypedConnectionList(typeof(T), id).ToList();
        ...
        public Dictionary<TKey, List<Action>> GetConnections()
        {
            var connections = _connections.ToDictionary(entry => entry.Key, entry => entry.Value.ToList());
        ...
        public Dictionary<TKey, Dictionary<Type, List<object>>> GetTypedConnections()
        {
            var typedConnections = _typedConnections.ToDictionary(entry => entry.Key, entry => entry.Value.ToDictionary(entry2 => entry2.Key, entry2 => entry2.Value.ToList()));
